...
import AlertModal from "@/components/molecules/Modals/AlertModal/AlertModal";
import Backdrop from "@/components/molecules/Modals/BackDrop/BackDrop";
      ...
      {isVisible && (
        <Backdrop onCancel={handleClickOutOfModal}>
          {modals.map((modal, index) => {
            ...
            return (
              <div
                className={`flex justify-center items-center absolute ${zIndex}`}
                key={modal.id}
              >
                {modal.modal}
              </div>
